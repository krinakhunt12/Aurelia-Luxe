        ...
        {filteredPieces.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {filteredPieces.map((product) => (
              <div key={product.id} className="group cursor-pointer animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="relative aspect-square overflow-hidden bg-[#FBFBFB] mb-6">
                  ...
                  <button 
                    ...
                    className={`absolute top-4 right-4 p-2 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 ${
                      isInWishlist(product.id) ? 'bg-white text-red-500 shadow-sm' : 'bg-white/0 text-black hover:bg-white'
                    }`}
                  >
                    <Heart 
                      size={18} 
